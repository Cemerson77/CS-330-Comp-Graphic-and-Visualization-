...
// 
// 
// This program creates a 3d scene of a gaming setup. The scene includes lighting,
// a TV, desk, xbox console, letter D light, and a xbox controller.
// 
//---------------------------------------------------
...
//window title
const char* const WINDOW_TITLE = "Christine Emerson";
...
// camera
Camera gCamera(glm::vec3(-6.0f, 6.0f, -7.0f), glm::vec3(0.0f, 1.0f, 0.0f), 45.0f, -20.0f);
...
glm::vec3 gSpotLightColor(0.0f, 1.0f, 0.0f);
glm::vec3 gSpotLightPosition(10.65f, -6.0f, 5.0f);
glm::vec3 gSpotLightScale(0.0f);

// Light color, position and scale
glm::vec3 gKeyLightColor(1.0f, 1.0f, 1.0f);
glm::vec3 gKeyLightPosition(1.0f, 6.0f, 0.0f);
...
//void UBuildHalfTorus(GLMesh& mesh, vector<float> properties, float radius, float length);
...
void main()
{
	...
	float keyImpact = max(dot(norm, keyLightDirection), 0.3);// Calculate diffuse impact by generating dot product of normal and light
	...
	fragmentColor = vec4(phong, textureColor.a); // Send lighting results to GPU, including the alpha component
...
void main()
{
	fragmentColor = vec4(1.0f); // Set color to white (1.0f,1.0f,1.0f) with alpha 0.0
}
);
...
void URender(vector<GLMesh> scene)
{
	...
	constexpr float angularVelocity = glm::radians(100.0f);

	if (gSpotLightOrbit)
	{
		glm::vec4 newPosition = glm::rotate(angularVelocity * gDeltaTime, glm::vec3(1.0f, 0.0f, 1.0f)) * glm::vec4(gSpotLightPosition, 10.0f);

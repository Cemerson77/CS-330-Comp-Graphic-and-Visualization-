...
{
	vector<float> c = { mesh.p[0], mesh.p[1], mesh.p[2], 0.0f };

	mesh.v = {
		0.5f,	0.0f,	0.5f,	c[0],	c[1],	c[2],	c[3],	0.0f,	0.5f,	// front left
		-0.5f,	0.0f,	0.5f,	c[0],	c[1],	c[2],	c[3],	0.0f,	0.5f,
		-0.5f,	1.0f,	0.5f,	c[0],	c[1],	c[2],	c[3],	0.0f,	1.0f,

		0.5f,	0.0f,	0.5f,	c[0],	c[1],	c[2],	c[3],	0.0f,	0.5f,	// front right
		0.5f,	1.0f,	0.5f,	c[0],	c[1],	c[2],	c[3],	0.0f,	1.0f,
		-0.5f,	1.0f,	0.5f,	c[0],	c[1],	c[2],	c[3],	0.0f,	1.0f,


		0.5f,	0.0f,	0.5f,	c[0],	c[1],	c[2],	c[3],	0.0f,	0.5f,	// right front
		0.5f,	1.0f,	0.5f,	c[0],	c[1],	c[2],	c[3],	0.0f,	1.0f,
		0.5f,	1.0f,	-0.5f,	c[0],	c[1],	c[2],	c[3],	0.5f,	1.0f,

		0.5f,	0.0f,	0.5f,	c[0],	c[1],	c[2],	c[3],	0.0f,	0.5f,	// right back
...
void ShapeBuilder::UBuildCircle(GLMesh& mesh)
{
	...
	{
		// triangle fan
		v.insert(v.end(), { 0.5f, 0.5f, 0.5f, c[0],	c[1], c[2],	c[3], 0.5f, 0.5f });		// origin (0.5, 0.5) works best for textures
...
void ShapeBuilder::UBuildLetterCube(GLMesh& mesh)
{
	{
		vector<float> c = { mesh.p[0], mesh.p[1], mesh.p[2], 0.0f };
		...
		mesh.v = {
			...
			//0.5f,	0.0f,	-0.5f,	c[0],	c[1],	c[2],	c[3],		0.5f,	0.5f,	// back left here down
			//-0.5f,	0.0f,	-0.5f,	c[0],	c[1],	c[2],	c[3],	0.75f,	0.5f,
			//-0.5f,	1.0f,	-0.5f,	c[0],	c[1],	c[2],	c[3],	0.75f,	1.0f,
...
void ShapeBuilder::UBuildCylinder(GLMesh& mesh)
{
	...
	//the calculated points are used to determine which section of
	// the texture to clamp to the corresponding point.
	constexpr float x = 0.0f;
	float j = 3.0f / (s / x);	// for calculating texture location; change 'x' to increase or decrease how many times the texture wraps around the cylinder
	float k = 300.0f;				// for texture clamping
...
void ShapeBuilder::UBuildPlane(GLMesh& mesh)
{
	...
	mesh.v = {
		-1.0f, 0.0f, -1.0f, c[0], c[1], c[2], c[3], 1.0f, 1.0f,	// 0
		 0.0f, 0.0f, 1.0f, c[0], c[1], c[2], c[3],  0.5f, 0.0f,	// 1
		- 1.0f, 0.0f, 1.0f, c[0], c[1], c[2], c[3],  1.0f, 0.0f,	// 2
